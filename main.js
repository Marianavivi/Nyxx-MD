"use strict";
process.env['NODE_TLS_REJECT_UNAUTHORIZED'] = '1'; // Disable TLS certificate validation (for development only)
import './config.js';
import './api.js';
...
import { readdirSync, statSync, unlinkSync, existsSync, readFileSync, watch, stat } from 'fs';
...
// Cache for message retries and user devices
const msgRetryCounterMap = new Map();
const msgRetryCounterCache = new NodeCache({ stdTTL: 0, checkperiod: 0 });
const userDevicesCache = new NodeCache({ stdTTL: 0, checkperiod: 0 });

// Polyfills for __filename, __dirname, and require
...
global.videoList = []; // Global array to store video information
global.videoListXXX = []; // Global array to potentially store adult video information (handle responsibly)
...
/* Credits to Otosaka (https://wa.me/51993966345) */

// ChatGPT database initialization with lowdb
...
// Code adapted for bot compatibility with the 8-digit code. Done by: https://github.com/GataNina-Li
let opcion
if (methodCodeQR) {
  opcion = '1'
}
...
console.info = () => { } // Suppress Baileys logs
...
key.remoteJid);
  ...
  //userDeviceCache: msgRetryCounterCache <=== who was the idiot????? 
};

global.conn = makeWASocket(connectionOptions);

// 8-digit code authentication handling
if (!fs.existsSync(`./${authFile}/creds.json`)) {
  if (opcion === '2' || methodCode) {
    opcion = '2'
    if (!conn.authState.creds.registered) {
      if (MethodMobile) throw new Error('Cannot use a pairing code with the mobile API')
...
// Periodic database writing and temp file cleaning
if (!opts['test']) {
  if (global.db) {
    setInterval(async () => {
...
/* That was the best joke in the world
        Although I wouldn't hesitate for a second
        But I don't regret having laughed
        Because fat is a feeling
        And that was the best joke in the world
        Although I wouldn't hesitate for a second
        that I regret being a fatso
        Because fat is a feeling
        - El waza 👻👻👻👻 (Aiden)            

   I can also make jokes Aiden...
        there goes the adjustment of the deleted ones
        smart sessions and sub-bots
        By (King Endymion 👺👍🏼) 

   None is better than tilin god
        - atte: sk1d             */
...
// Function to delete core.<number> files
...
function purgeSessionSB() {
  try {
    ...
    console.log(chalk.bold.red(`[ ℹ️ ] Something went wrong during the deletion, files not deleted`))
  }
}
 ...
 if (code && code !== DisconnectReason.loggedOut && conn?.ws.socket == null) {
  ...
  if (connection === 'close') {
    if (reason === DisconnectReason.badSession) {
      conn.logger.error(`[ ⚠ ] Bad session, please delete the folder ${global.authFile} and scan again.`);
      //process.exit();
    } else if (reason === DisconnectReason.connectionClosed) {
      conn.logger.warn(`[ ⚠ ] Connection closed, reconnecting...`);
      await global.reloadHandler(true).catch(console.error);
    } else if (reason === DisconnectReason.connectionLost) {
      conn.logger.warn(`[ ⚠ ] Connection lost to the server, reconnecting...`);
      await global.reloadHandler(true).catch(console.error);
...
// Handler for messages and events
...
global.reloadHandler = async function (restatConn) {
  ...
  // To change these messages, just the files in the language folder, 
  // look for the key "handler" inside the json and change it if necessary
  conn.welcome = '👋 Welcome!\n@user';
  conn.bye = '👋 Goodbye!\n@user';
  conn.spromote = '*[ ℹ️ ] @user was promoted to admin.*';
  conn.sdemote = '*[ ℹ️ ] @user was demoted from admin.*';
  conn.sDesc = '*[ ℹ️ ] Group description has been changed.*';
  conn.sSubject = '*[ ℹ️ ] Group subject has been changed.*';
  ...
  if (pluginFilter(filename)) {
    ...
    if (err) conn.logger.error(`syntax error while loading '${filename}'\n${format(err)}`);
      ...
      try {
        ...
        global.plugins
       = Object.fromEntries(Object.entries(global.plugins).sort(([a], [b]) => a.localeCompare(b)));
      }
    }
  }
};
...
// Quick test for external tool availability
...
// Periodic temporary file cleaning
setInterval(async () => {
  if (stopped === 'close' || !conn || !conn?.user) return;
  await clearTmp();
}, 180000);

// Periodic session and old file purging (adjust interval as needed)
/*
setInterval(async () => {
  if (stopped === 'close' || !conn || !conn?.user) return; 
...
// Update bot's profile status with uptime
setInterval(async () => {
  if (stopped === 'close' || !conn || !conn?.user) return;
